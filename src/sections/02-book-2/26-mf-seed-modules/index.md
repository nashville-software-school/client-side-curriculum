...
This is the third time you've built this kind of boundary, after Surf Shop's `data.js`/`functions.js` split and Martin's Aquarium's `aquariumData.js`. You now have six separate seed modules, and `main.js` only imports the one function it's testing at a time, by name. Why not put all six `createX` functions in one big `seeds.js` file instead? What would `main.js` have to import differently if you did?
