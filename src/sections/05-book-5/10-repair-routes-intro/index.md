...
<a href="https://www.youtube.com/embed/IIb47gZBFbY?start=49&autoplay=1" target="yt-ex10">[0:49]</a> first thing we need to do is after installing react router Dom we need to wrap our app component with browser router so browser router is an is a component that we get from react router Dom so we need to import that all right so now I'm going to wrap the app component with browser router now what browser router does is it keeps our application in sync with the URL we need this because we want to render different components depending on which URL the user visits
